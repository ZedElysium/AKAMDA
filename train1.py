...
def Train(directory, epochs, n_classes, in_size, out_dim, dropout, slope, lr, wd, random_seed, cuda):
   ...
   # print('## vertices:', g.number_of_nodes())
    ...
    for train_idx, test_idx in kf.split(samples[:, 2]):     # 返回训练集和测试集的索引train：test 4:1
        ...
        g_train = dgl.edge_subgraph(g, train_eid, relabel_nodes=False)
        ...
        for epoch in range(epochs):
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                # 执行第一次前向传播
                score_train1 = model(g_train, src_train, dst_train, True)
                # 执行第二次前向传播，确保dropout模式激活
                score_train2 = model(g_train, src_train, dst_train, True)

                # 计算主损失
                loss_train1 = cul_loss(score_train1, label_train, model)
                loss_train2 = cul_loss(score_train2, label_train, model)

                # 计算一致性损失
                consistency_loss = F.kl_div(F.log_softmax(score_train1, dim=1), F.softmax(score_train2, dim=1),
                                            reduction='batchmean') + \
                                   F.kl_div(F.log_softmax(score_train2, dim=1), F.softmax(score_train1, dim=1),
                                            reduction='batchmean')

                # 总损失
                consistency_weight = 0.4
                loss_train = loss_train1 + loss_train2 + consistency_weight * consistency_loss
                ...
                #
                # # 获取两个独立的dropout掩码
                # dropout_mask1 = torch.bernoulli(torch.full(score_train.shape, 0.5)).to(score_train.device)
                # dropout_mask2 = torch.bernoulli(torch.full(score_train.shape, 0.5)).to(score_train.device)
                #
                # # 执行两次前向传播
                # out1 = model(score_train, dropout_mask1)
                # out2 = model(score_train, dropout_mask2)
                #
                # # 计算一致性损失（KL散度）
                # consistency_loss = F.kl_div(out1, out2.exp(), reduction='batchmean') + F.kl_div(out2, out1.exp(),
                #                                                                                 reduction='batchmean')
                #
                # # 计算主损失
                # loss = F.nll_loss(out1, label_train) + F.nll_loss(out2, label_train) + consistency_loss
                #
                # loss.backward()
                # optimizer.step()
            ...
            score_train_avg = (score_train1 + score_train2) / 2
            score_train_cpu = np.squeeze(score_train_avg.cpu().detach().numpy())
